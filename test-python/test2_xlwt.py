...
import cx_Oracle
con = cx_Oracle.connect('test50overitcommesse/test50overitcommesse@test-oracle-11r2/testl112.overit.it')
cur = con.cursor()
v_query = 'SELECT * fROM ('
v_query += ' select                                                                  '
v_query += ' X10ROCDID,                                                              '
...
result = cur.execute(v_query)
#for result in cur:
# print result
v_num_rows = len(result)
v_num_cols = len(result[0])
...
v_description_lentgh = len(cur.description)
